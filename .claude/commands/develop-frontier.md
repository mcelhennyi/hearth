...
description: >-
  ...
  ticket to complete TEST→DEV→VAL in separate worktrees, then runs finish-feature
...
End-to-end: discover parallel-capable tickets (**global** graph — may span **multiple** **`FR-NNNN`** features per **`docs/ai-context.md` §2c**), run one subagent per ticket in a dedicated worktree, execute **TEST → DEV → VAL** serially per ticket, then run **`finish-feature`** (feature branch → **PR to `main`**) **or** **`finish-frontier`** (merge into **`main`**) per **`docs/ai-context.md` §2d**.
...
- **Feature-branch workflow:** feature integration branch **`feat/FR-NNNN-<slug>`** exists (or will be created before ticket PRs). **Direct-to-main:** integration checkout on **`main`** is available for **`finish-frontier`**.
...
1. Update **`tasks/ticket-progress.md`** `Current focus` for multi-ticket work:
   ...
   - **Next agent should**: frontier ticket ids and `worktrees/<slug>/` paths
...
- Use only its worktree (`worktrees/<slug>/`, branch `feat/T-FR-NNNN-xx-...`).
...
- **Default for a single `FR-NNNN` product line:** run **`finish-feature`** — merge ticket branches into **`feat/FR-NNNN-<slug>`**, revalidate there, push the feature branch, open **PR → `main`**. Do **not** push **`main`** from automation.
