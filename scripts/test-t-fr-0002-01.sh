...
for _ in {1..40}; do
  if curl -ksS --resolve hearth.local:443:127.0.0.1 "$STACK_URL" | grep -q "$KNOWN_BODY"; then
    break
  fi
  sleep 1
done

HTML="$(curl -ksS --resolve hearth.local:443:127.0.0.1 "$STACK_URL")"
echo "$HTML" | grep -q "$KNOWN_BODY"
...
grep -q "BEGIN CERTIFICATE" /tmp/t-fr-0002-01-ca.crt
