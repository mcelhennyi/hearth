...
| **`hearth-users` plugin** | First-admin setup, username/password login UI, password hashes, user/session CRUD, `/verify` for edge |
...
|---------|------|-------------------|--------|
| `GET/POST /hearth-users/login` | HTTP (plugin UI) | First-admin setup when no account exists; username/password login otherwise; `POST` sets session cookie | `hearth-users` |
...
| `spark.call("hearth-users", …)` | Spark | `session.current`, admin user-management operations, `provider.status` | `hearth-users` |
...
|-------|--------|---------|
| Local accounts: username, display name, roles, disabled flag, password hash | Session cookie (`HttpOnly`, `Secure`, `SameSite=Lax`) | `var/hearth/plugins/hearth-users/` (plugin-owned SQLite) |
