...
| FR-0003 | `FR-0003-hearth-pi-docker-cli/` | `design` | **`T-FR-0003-01`** … **`T-FR-0003-13`** in [`FR-0003-hearth-pi-docker-cli/tickets.md`](FR-0003-hearth-pi-docker-cli/tickets.md) | Raspberry Pi (and similar) **Docker Compose** install story: `./install` bootstrap, `hearth` admin CLI (plugin lifecycle, updates, compose control), per-plugin `plugin` CLI + Kindling hooks; lays groundwork for later admin UI parity. |
