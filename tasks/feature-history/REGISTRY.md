...
| FR-0003 | `FR-0003-hearth-pi-docker-cli/` | `in-progress` | **`T-FR-0003-01`** … **`T-FR-0003-13`** in [`FR-0003-hearth-pi-docker-cli/tickets.md`](FR-0003-hearth-pi-docker-cli/tickets.md) | **Implementation active** on **`feat/FR-0003-hearth-pi-docker-cli`** (2026-05-10). Supersedes scheduling-only “park until FR-0002” (option A); no **`T-FR-0003-xx`** dependency on FR-0002 — hub/Mantle integration uses stubs until the hub stack exists. Docker Compose Pi install, `./install`, `hearth` + per-plugin `plugin` CLI. |
