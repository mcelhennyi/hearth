...
| Q4 | Auth in MVP: basic auth required vs no-auth on LAN | **Auth required**; `HEARTH_TRUST_LAN=1` to opt out | open → **DG-I1** (superseded by **FR-0004** when resumed) |
...
| Q11 | Where does VAPID keypair live and rotate? | `var/hearth/secrets/vapid.{pub,priv}`; rotation invalidates subscriptions, deferred to Phase 2 | open → **DG-N1** |
