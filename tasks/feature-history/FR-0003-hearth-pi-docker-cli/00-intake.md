...
| **Constraints** | Install tree **`<install-dir>/hearth`**; minimal top-level clutter (data in subdirs); Pi + Docker Compose; subsequent updates via **`hearth --update`**; Kindling plugins get **`install`** + **`plugin`**; mirror **CLI parity** rule in Cursor + Claude docs |
| **Success definition** (1–3 bullets) | (1) Fresh clone on Pi: **`./install`** prepares the host (including Docker where required), lays out **`hearth/`** with README + version manifest, generates/merges Compose for hub + **enabled** plugins, and starts the stack. (2) **`hearth`** exposes the requested plugin and stack commands plus a small set of high-value extras (status, logs, doctor, compose passthrough). (3) Each plugin dir has **`plugin`** supporting the listed lifecycle commands, **`plugin --exit`**, and passthrough admin subcommands; templates/docs updated so new Kindling plugins inherit the pattern. |
...
- Per-plugin: **`install`** script (once), **`plugin`** at **`<install-dir>/hearth/plugins/<slug>/plugin`**, README + version doc like the root.
...
**Naming (amended 2026-05-16):** Docker-profile install tree is **`<install-dir>/hearth/`** (same basename as the product). Early intake text said **`heart/`**; that was a naming mistake — corrected in **`docs/design/deployment.md`** (amendment **HRT-DEP-001**) and implementation.
