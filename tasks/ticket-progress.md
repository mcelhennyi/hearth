...
| Field | Value |
|-------|--------|
| **Active ticket** | `T-FR-0004-03` |
| **Active phase** | TEST → DEV → VAL |
| **Branch / worktree** | `feat/FR-0004-centralized-users-auth` @ `.worktrees/FR-0004-centralized-users-auth/feature/`; next ticket worktree `.worktrees/FR-0004-centralized-users-auth/T-FR-0004-03-users-session-verify/` |
| **Session status** | `developing` |
| **Next agent should** | Continue FR-0004 frontier with `T-FR-0004-03` (password, session, verify API) on `feat/FR-0004-centralized-users-auth-T-FR-0004-03-users-session-verify`, then merge to `feat/FR-0004-centralized-users-auth` and identify the next wave. |
...
**FR-0004** resumed — `T-FR-0004-02` merged into `feat/FR-0004-centralized-users-auth`; `T-FR-0004-03` is the next frontier ticket.
...
| T-FR-0004-03 | Users plugin: password, session, verify API | done | done | done | `FR-0004`. Plugin-owned SQLite, Argon2id setup, secure session cookie, login/logout, `/api/session`, and `/api/verify` claims implemented. Validation: `./develop test` 251 passed, 3 skipped; focused login-cookie/verify vectors 2 passed. |
