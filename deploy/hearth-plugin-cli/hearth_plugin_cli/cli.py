...
from hearth_install.plugin_session import FROM_ENV, PluginEnterSessionError, pop_enter_target
...
def run_plugin_cli(
    ...
    *,
    ...
    if argv[0] == "--exit":
        try:
            target = pop_enter_target(src_env)
        except PluginEnterSessionError as exc:
            previous = src_env.get(FROM_ENV, "").strip()
            if not previous:
                print(f"plugin: {exc}", file=err)
                return 1
            target = Path(previous)
        if not target.is_dir():
            print(f"plugin: enter target is not a directory: {target}", file=err)
            return 1
        print(f"cd {shlex.quote(str(target.resolve()))}", file=out)
        return 0
