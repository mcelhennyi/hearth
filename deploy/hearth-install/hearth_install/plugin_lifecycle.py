...
def set_plugin_enabled(hearth: Path, slug: str, *, enabled: bool) -> None:
    ...
    if not found:
        msg = f"plugin {slug!r} is not listed in {hearth / 'state' / 'plugins.yaml'}"
        raise PluginLifecycleError(msg)
    save_plugin_registry(hearth, new_rows)
    try:
...
def remove_plugin_from_install(hearth: Path, slug: str) -> None:
    ...
    plugin_dir = hearth / "plugins" / slug
    if plugin_dir.is_dir():
        shutil.rmtree(plugin_dir)

    var_plugin = hearth / "var" / "plugins" / slug
