...
If **`Codex.project.md`** exists at the repository root, load it **after** this file for Codex-only extensions. That file is **not** overwritten by **`./sync-skeleton`**. See **`docs/skeleton-project-overlays.md`**.
...
Codex discovers reusable command skills under **`.agents/skills/`**. The skeleton ships migrated source-command skills named **`source-command-*`** so prompts like **`/feature-request`**, **`/identify-frontier`**, or **`/add-todo`** can resolve to the same workflow that Cursor and Claude use.
...
Development standards: **`.claude/rules/development-standards.md`** plus Codex session rules in **`AGENTS.md`** and **`docs/ai-context.md`**.
