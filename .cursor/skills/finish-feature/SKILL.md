...
name: finish-feature
description: >-
  Merges all ticket branches for one FR-NNNN feature into the feature integration
  branch, validates, and opens (or updates) a PR to main for human review. Never
  deletes remote branches automatically. Use after develop-frontier when using
  the feature-branch workflow.
---

# Finish feature (`FR-NNNN`)

Close **implementation** for a **single** product feature that used a **feature integration branch** — all ticket branches **`feat/T-FR-NNNN-xx-*`** should already merge **into** **`feat/FR-NNNN-<slug>`** (not directly to `main`). This skill finishes that line and hands off to humans on **`main`**.

## Preconditions

- **`FR-NNNN`** and **`<slug>`** known; **`tasks/feature-history/FR-NNNN-<slug>/`** exists with **`tickets.md`**, diaries, and **`handoffs/`** as needed.
- **Feature integration branch** exists on the remote, e.g. **`feat/FR-0007-auth-overhaul`**. Ticket branches exist and are pushed.
...
In a checkout of **`feat/FR-NNNN-<slug>`** (dedicated worktree recommended):

1. `git fetch origin`
2. `git checkout feat/FR-NNNN-<slug>` and `git pull --ff-only` (or merge) as appropriate.
3. For each **`feat/T-FR-NNNN-xx-*`** branch in **dependency-safe** order, `git merge --no-ff <branch> -m "merge: T-FR-NNNN-xx (finish feature FR-NNNN)"`.
4. Resolve conflicts; prefer preserving both intents. Do **not** delete the source ticket branches locally or on the remote.
...
- **Never** automatically **`git push origin --delete`** or **`git branch -D`** ticket or feature branches used in this workflow — they are the **audit trail** of how work evolved.
...
| Skill | When |
|-------|------|
| **`finish-feature`** | One **`FR-NNNN`**: ticket branches → **feature branch** → **PR to `main`** for human merge. |
