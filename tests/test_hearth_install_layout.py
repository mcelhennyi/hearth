"""Contract tests for ``hearth/`` layout and ``VERSION.json`` (T-FR-0003-02)."""
...
def _expected_subdirs(hearth: Path) -> tuple[str, ...]:
    return ("compose", "plugins", "state", "var", "bin")


def test_ensure_hearth_layout_creates_directories_and_readme(tmp_path: Path) -> None:
    root = tmp_path / "deploy"
    hearth = ensure_hearth_layout(root, hearth_ref="abc123")
    assert hearth == (root / "hearth").resolve()
    for name in _expected_subdirs(hearth):
        assert (hearth / name).is_dir()
    assert (hearth / "README.md").is_file()
    text = (hearth / "README.md").read_text(encoding="utf-8")
    assert "VERSION.json" in text
    assert (hearth / "VERSION.json").is_file()


def test_version_json_parse_round_trip(tmp_path: Path) -> None:
    ensure_hearth_layout(tmp_path, hearth_ref="ref-for-test")
    manifest = read_version_manifest(tmp_path / "hearth" / "VERSION.json")
    assert manifest.schema == 1
    assert manifest.hearth_ref == "ref-for-test"


def test_ensure_hearth_layout_idempotent_version_preserved(tmp_path: Path) -> None:
    ensure_hearth_layout(tmp_path, hearth_ref="first")
    first = (tmp_path / "hearth" / "VERSION.json").read_text(encoding="utf-8")
    ensure_hearth_layout(tmp_path, hearth_ref="second")
    second = (tmp_path / "hearth" / "VERSION.json").read_text(encoding="utf-8")
    assert first == second
